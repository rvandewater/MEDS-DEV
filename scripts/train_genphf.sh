...
#SBATCH -C '(GPU_SKU:A40|GPU_SKU:V100|GPU_SKU:2080Ti)&ARCH:X86'
...
for dataset in "${datasets[@]}"; do
    ...
    for task in "${tasks[@]}"; do
        ...
        "--config-name=genhpf_train" || {
                    echo "Error: genhpf-train failed for task $TASK_NAME in dataset $DATASET_NAME. Skipping to the next task."
                    continue
                }
