...
#SBATCH -C '(GPU_SKU:A40|GPU_SKU:V100)&ARCH:X86'
...
export datasets=("HIRID" "MIMIC-IV" "eICU") #("AUMCdb" "EHRShot" "HIRID" "INSPIRE" "NWICU" "SICdb")
export MODEL_NAME="genhpf"
...
for dataset in "${datasets[@]}"; do
    ...
    for task in "${tasks[@]}"; do
        ...
        debug_print_env
        if [ ! -f "$FINETUNED_MODEL_DIR/checkpoints/checkpoint_best.pt" ]; then
        ...
        genhpf-test \
            ...
            "checkpoint.load_checkpoint=$FINETUNED_MODEL_DIR/checkpoints/checkpoint_best.pt" \
            "dataset.test_subset=held_out" \
            "common.debug=False" \
            "--config-dir=/sc/home/robin.vandewater/projects/MEDS_DEV_NEW/src/MEDS_DEV/models/genhpf" \
            "--config-name=genhpf_predict"
      mv $OUTPUT_DIR/held_out.parquet $OUTPUT_DIR/predictions.parquet
    done
done
